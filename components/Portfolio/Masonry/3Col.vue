        ...
        <div class="filtering col-12 mb-80 text-center">
          <div class="filter">
            ...
            <span v-for="(item, index) in data.filters" :data-filter="item.filter" :class="index === 0 ? 'active' : ''"
              :data-count="item.count" :key="item.id">{{ item.title }}</span>
          </div>
        </div>
      </div>
      <div class="gallery">
        <div class="row masonry md-marg">
          <div v-for="(item) in data.gallery" :class="`col-lg-4 col-md-6 items ${item.filter} info-overlay mb-50`"
            :key="item.id">
...
//= Data
import data from '@/data/Portfolio/masonry-3.json'
