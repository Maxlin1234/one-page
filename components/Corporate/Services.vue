        ...
        <div class="col-lg-4" v-for="item in data" :key="item.id">
          <div class="serv-item md-mb50 radius-5">
            <div class="icon-img-50 mb-40">
              <img :src="`/light/${item.image}`" alt="" v-if="lightMode" />
              <img :src="`/dark/${item.image}`" alt="" v-else />
            </div>
            <span class="mb-10 opacity-7">{{ item.category }}</span>
            <h6 class="mb-15">{{ item.title }}</h6>
            <p class="fz-14">{{ item.text }}</p>
            <div class="underline">
              <NuxtLink to="/dark/page-services" class="mt-30 ls1 fz-12 text-u">Read More
